...
def manage_users_get(client, admin_user):
    """
...
def manage_users_post_invalid_data(client, admin_user):
    """
...
def manage_users_post_valid_data(client, admin_user, normal_user):
    """
...
def manage_users_post_ban_user(client, admin_user, normal_user):
    """
...
def manage_users_post_unban_user(client, admin_user, normal_user):
    """
...
def auth_register_missing_fields(client):
    response = client.post('/auth/sign-in', data={
        'form_type': 'register',
        'username': '',
        'email': 'newuser@example.com',
...
def auth_register_existing_email(client, user):
    response = client.post('/auth/sign-in', data={
        'form_type': 'register',
        'username': 'newuser',
        'email': user.email,
...
def account_update_profile(client, user):
    client.post('/auth/sign-in', data={'form_type': 'login', 'email': user.email, 'password': 'password'})
...
def account_change_password(client, user):
    client.post('/auth/sign-in', data={'form_type': 'login', 'email': user.email, 'password': 'password'})
    response = client.post('/account', data={
...
def checkout_empty_cart(client, user):
    response = checkout(client, user)
    assert response.status_code == 302
    assert b'Your cart is empty.' in response.data

def checkout_success(client, user, cart_items):
    response = checkout(client, user)
    assert response.status_code == 302
...
def place_order_missing_seller(client, user, card_without_seller):
    client.post(url_for('auth.login'), data={'email': user.email, 'password': 'password'})
...
def submit_feedback_success(client, user, order):
    client.post(url_for('auth.login'), data={'email': user.email, 'password': 'password'})
...
def submit_feedback_unauthorized(client, user, other_user_order):
    client.post(url_for('auth.login'), data={'email': user.email, 'password': 'password'})
...
def upload_card_missing_details(client, admin_user):
    client.post(url_for('auth.login'), data={'email': admin_user.email, 'password': 'password'})
    response = client.post(url_for('seller.upload_card'), data={
        'name': '',
        'follow_tcg': 'on',
...
def upload_card_name_mismatch(client, admin_user):
    client.post(url_for('auth.login'), data={'email': admin_user.email, 'password': 'password'})
    response = client.post(url_for('seller.upload_card'), data={
        'name': 'Charizard',
        'follow_tcg': 'on',
...
def get_card_details_not_found(client, admin_user):
    client.post(url_for('auth.login'), data={'email': admin_user.email, 'password': 'password'})
...
def report_user_success(client, admin_user, normal_user):
...
def report_user_missing_reason(client, admin_user, normal_user):
    client.post(url_for('auth.login'), data={'email': normal_user.email, 'password': 'password'})
