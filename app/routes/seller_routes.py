...
def upload_card():
    ...
    if request.method == "POST":
        name, follow_tcg, price = request.form.get("name"), request.form.get("follow_tcg") == "on", float(request.form.get("price") or 0.0)
        condition, set_name, number = request.form.get("condition"), request.form.get("set_name"), request.form.get("number")
        card_type, is_graded = request.form.get("card_type"), request.form.get("is_graded") == "on"
        grade, grading_company = request.form.get("grade") if is_graded else None, request.form.get("grading_company") if is_graded else None
        language, file, back_file = request.form.get("language"), request.files.get("image"), request.files.get("back_image")
        ...
        if language == "en":
            try:
                ...
                filtered_cards = [card for card in card_data if card.get("set", {}).get("name", "").strip().lower() == set_name.strip().lower() and str(card.get("number", "")).strip().lower() == str(number).strip().lower()]
        ...
        back_image_url = upload_to_s3(back_file, bucket_name=Config.S3_BUCKET) if back_file else None
...
def get_card_details():
    ...
    """
    language, set_name, number = request.args.get("language", "en"), request.args.get("set_name"), request.args.get("number")
...
def seller_dashboard():
    ...
    if search_query:
        query = query.join(Order.buyer).filter(User.username.ilike(f"%{search_query}%") | Order.cards.any(Card.name.ilike(f"%{search_query}%")))
    ...
    for order in pending_orders:
        ...
        orders_with_details.append({"id": order.id, "buyer": order.buyer, "created_at": order.created_at, "cards": cards, "total_price": total_price})
    ...
    for order in completed_orders_query:
        ...
        total_price = sum(card.price for card in cards)
        completed_orders.append({"id": order.id, "buyer": order.buyer, "created_at": order.created_at, "cards": cards, "total_price": total_price})
    ...
    return render_template("seller_dashboard.html", orders=orders_with_details, completed_orders=completed_orders, stats=stats)
